...
author: 
 ...
 - name: Claude Grasland
   affiliation: FR CIST, UMR Géographie-Cités
image: 
output:
  rzine::readrzine:
    highlight: kate
...
<li>**Temps de lecture estimé** : 50 minutes (56 000 caractères). </li>
</ul>
...
Cet outil a bénéficié d'une importante reconnaissance autour de 2010, et plusieurs versions de cet outil ont été implémentées dans divers projets européens, pour notamment permettre l'analyse des disparités régionales sur des indicateurs socio-économiques à l'échelle de l'Union Européenne [@espon2004], [@ep2007], ([ESPON, 2011](https://www.espon.eu/tools-maps/espon-hyperatlas){target="_blank"}). Cet outil a aussi suscité l'intérêt dans l'Éducation Nationale, notamment pour proposer aux élèves de 1ère un cadre méthodologique adapté pour *enseigner les disparités socio-spatiales au sein de l'UE* ([Éducation Nationale, 2014](https://www.ac-paris.fr/portail/jcms/p1_986484/enseigner-les-disparites-socio-spatiales-au-sein-de-l-ue-avec-hyperatlas-1ere){target="_blank"}). 
...
Le groupe de recherche HyperCarte, actif entre 1996 et 2015, reposait sur quatre équipes de recherche en géographie ([UMS RIATE](https://riate.cnrs.fr/){target="_blank"} et [Géographie-Cités](https://www.parisgeo.cnrs.fr/){target="_blank"}) et en informatique ([LIG-STeamer](http://steamer.imag.fr/){target="_blank"} et [LIG-MESCAL](http://mescal.imag.fr/){target="_blank"}). 
...
**La préparation des données et géométries n'a pas vocation à être rejouée pour lancer l'analyse. Elle détaille en revanche les étapes de préparation à suivre pour construire un jeu de données opérationnel à partir des fichiers de référence distribués par l'INSEE et l'IGN.** Cette partie se conclue par l'export de couches géographiques enrichies au format geopackage, qui sera importé en tout début de la prochaine partie.

Ce script peut en revanche utile si l'on souhaite préparer des données pour d'autres indicateurs ou d'autres mailles territoriales.
...
- `cartography`, pour le calcul des discontinuités.
- `RColorBrewer`, pour les palettes de couleur. 
- `readxl`, pour manipuler des fichiers Excel.
- `flextable`, pour le formatage des tableaux de synthèse. 
...
Cette sous-partie vise à extraire des fichiers source de l'IGN et de l'INSEE les maillages territoriaux utiles pour l'analyse et les espaces d'étude d'intérêt : découpages en IRIS^[__IRIS__: Plus petit découpage territorial en France, qui constitue la brique de base en matière de diffusion de données infra-communales par l'INSEE. Les communes d'au moins 10 000 habitants et une forte proportion des communes de 5 000 à 10 000 habitants sont découpées en IRIS. On distingue les IRIS d'habitat, dont la population se situe en général entre 1 800 et  5 000 habitants et dont les limites s'appuient sur les grandes coupures du tissu urbain, des IRIS d'activité (plus de 1 000 salariés et des IRIS divers (parcs, forêts, eaux, etc.)], communes et EPCI pour la Métropole du Grand Paris et les 21 autres métropoles françaises (afin de pouvoir éventuellement étendre à l'analyse à d'autres espaces d'étude).

<p class="center"> **[<img src="figure/zip.png" width="50"/> Accéder aux données source](https://gitlab.huma-num.fr/rysebaert/mta_rzine/-/tree/master/input)** </p>  

Cette extraction est effectuée sur plusieurs fichiers de référence stockés dans l'archive ci-dessus. Ils sont distribués et mis à jour annuellement par l'INSEE et l'IGN. Il s'agit de fichiers bruts, non modifiés au préalable :

**1) CONTOUR-IRIS.shp** : Les géométries des IRIS de France métropolitaine et leur commune d'appartenance, au découpage du 1er janvier 2019, fournies par [l'IGN (ADMIN-EXPRESS-COG édition 2019 par territoire)](https://geoservices.ign.fr/documentation/diffusion/telechargement-donnees-libres.html#contoursiris){target="_blank"}. 

**2) table-appartenance-geo-communes-19.xls** : L'appartenance des communes à leur EPCI de rattachement et le type de coopération (communautés de communes, communautés d'agglomération, communautés urbaines, métropoles) au 1er janvier 2019. Ce fichier est distribué par [l'INSEE](https://www.insee.fr/fr/information/2028028){target="_blank"}. 

**3) metropoles.xls** : Un fichier construit par nos soins, à partir des sites Web de référence des métropoles, et qui vise à identifier des sous-ensembles territoriaux cohérents à l'intérieur de ces métropoles. Dans le cadre de la métropole du Grand Paris, il s'agit des 11 communautés d'agglomération (+ Paris) qui la constitue. Pour les autres métropoles, l'importance des EPCI intermédiaires est moins prépondérant, puisque les métropoles ont vocation à remplacer les anciens EPCI. Nous avons néanmoins restitué quand c'était possible la géographie de ces anciennes mailles de décision afin de disposer d'un découpage territorial intermédiaire entre les communes et les métropoles. Quand ce maillage intermédiaire était inexistant (transformation de communautés urbaines en métropole sans inclusion de nouvelles communes au périmètre de coopération), nous avons uniquement fait la distinction entre la ville centre (LIB_EPCI = C) et sa périphérie (LIB_EPCI = P). 
...
Le script ci-dessous importe ces trois fichiers, affecte à chacun des IRIS un code/libellé communal et d'EPCI d'appartenance. Ne sont retenus que les IRIS qui sont inclus dans une des 22 métropoles françaises. Les IRIS sont ensuite fusionnés par commune, EPCI, métropole puis exportés au format geopackage. Ce geopackage contient quatre couches géographiques : celle des IRIS, celle des communes, celles des EPCI intermédiaires, et celles des métropoles. Elles sont enregistrées dans le fichier `input.gpkg` pour éviter d'avoir à rejouer ce script de préparation, un peu volumineux en temps de calcul. 

```{r prepare_geom, echo = TRUE, eval= TRUE, message=FALSE, warning=FALSE}
...
# Combiner ces dataframes d'entrée
...
Voici le dénombrement de objets géographiques disponible par découpage territorial et métropole.

```{r geographical_objects, echo = TRUE, eval = TRUE, message=FALSE, warning=FALSE}
## Synthèse (table)
...
Et l'illustration de l'emprise spatiale de ces découpages territoriaux pour la Métropole du Grand Paris. Les fonctions de représentations cartographiques utilisées dans cette fiche sont issues du package `mapsf`, dont l'usage et la documentation sont très bien détaillés sur ce [site Web](https://riatelab.github.io/mapsf/). L'objet de cette fiche n'étant pas de traiter de la cartographie thématique, nous ne détaillerons pas outre mesure ces fonctions de représentation.  

```{r layer_presentation, echo = TRUE, eval = TRUE, message=FALSE, warning=FALSE}
# Sélection de la Métropole du Grand Paris
Sel <- "Métropole du Grand Paris" 
tmp <- com[com$LIB_EPCI ==  Sel,]
tmp2 <- iris[iris$LIB_EPCI ==  Sel,]
tmp3 <- epci[epci$LIB_EPCI ==  Sel,]
tmp4 <- metro[metro$LIB_EPCI ==  Sel,]

# Theme cartographique
mf_theme(bg = NA, fg = NA, mar = c(0, 0, 0, 0), tab = FALSE, 
         pos = "left", inner = TRUE, line = 1.3, cex = 1, font = 2)

# Couches géographiques
mf_init(tmp4, expandBB = c(0,0.2,0,0)) # Initialiser l'emprise de la carte (Métro du Grand Paris)
mf_shadow(tmp4, col = "#616161", cex = 0.5, add = TRUE) # Ombrage 
mf_map(tmp2, col = "peachpuff", border = "lightgrey", lwd = 0.2, add = TRUE) # IRIS
mf_map(tmp, col = NA, border = "black", lwd = 0.5, add = TRUE) # Communes
mf_map(tmp3, col = NA, border = "darkred", lwd = 1, add = TRUE) # EPCI
mf_map(tmp4, col = NA, border = "black", lwd = 2, add = TRUE) # Métropole 

# Légende
legend("left", legend=c("Métropole", "EPCI", "Communes", "IRIS"),
       col=c("black", "darkred", "black", "lightgrey"), lty = 1,
       lwd = c(2, 1, 0.5, 0.2), box.lty = 0, cex = 0.8)

# Habillage
mf_title("\n\nDécoupages territoriaux créés à l'issue de la préparation des géométries",
         fg = "black", cex = .9)
...
Nous importons ici le fichier ***base-cc-emploi-pop-active-2016.xls*** mis à disposition par [l'INSEE](https://www.insee.fr/fr/statistiques/4171446?sommaire=4171473){target="_blank"}. Il contient les données communales pour l'emploi au lieu de travail et la population active occupée en 2016, fournies par . Ce jeu de données n'est pas disponible à l'échelle des IRIS.

Le second fichier de données, ***base-ic-logement-2016.xls***, aussi distribué par l'[INSEE](https://www.insee.fr/fr/statistiques/4228432){target="_blank"} à l'échelle des IRIS, permet de disposer d'indicateurs concernant le niveau de motorisation des ménages. Il ne sera utilisé qu'en toute fin de document pour montrer la possible transposition à d'autres espaces d'étude et d'autres mailles territoriales.

Les données et géométries utiles sont exportées dans le fichier `input.gpkg` afin d'éviter d'avoir à rejouer le script de préparation des données. 
```{r attribute_data, echo = TRUE, eval= FALSE, message=FALSE, warning=FALSE}
# Population active [COMMUNES]
...
# Sauver les données au format geopackage
...
# Import
com <- st_read("data/input.gpkg", layer = "com", quiet = TRUE) 
epci <- st_read("data/input.gpkg", layer = "epci", quiet = TRUE) 
metro <- st_read("data/input.gpkg", layer = "metro", quiet = TRUE) 

# Filtrer sur Paris
com <- com[com$LIB_EPCI == "Métropole du Grand Paris",]
epci <- epci[epci$LIB_EPCI == "Métropole du Grand Paris",]
metro <- metro[metro$LIB_EPCI == "Métropole du Grand Paris",]
```


L'objet `com` contient toutes les informations nécessaires à la réalisation de l'analyse : des identifiants géographiques (code communal et nom de la commune), la métropole d'appartenance de la commune (variable METRO), l'identifiant et le nom de l'EPCI d'appartenance (LIB_EPCI_SUB et POS), des données attributaires (P16_EMPLT et C16_ACTOCC15654), un dénominateur et les géométries communales. 
...
Cette fiche contient un nombre important de représentations cartographiques, toutes réalisées avec le package [`mapsf`](https://riatelab.github.io/mapsf/). La fonction `mta_theme` est créée pour automatiser la mise en page et alléger le code. Elle fait appel aux fonctions `mf_theme`, `mf_scale` et `mf_title` du package `mapsf`. On rend possible l'affichage ou non de l'échelle et des sources pour les illustrations qui regroupent plusieurs représentations cartographiques. L'argument `double = TRUE` est utilisé pour optimiser l'affichage de deux cartes en vis-à-vis. 
...
```{r template_carto, eval=TRUE, echo = TRUE}
...
```{r template_table, eval=TRUE, echo = TRUE}
...
mf_map(type = "prop", x = com, var = "denom", col = "#515FAA", border = "white",
       leg_pos = "bottomleft", leg_val_rnd = -2, inches = 0.15,
       leg_title = "Nombre d'actifs\nau lieu de résidence, 2016", add = TRUE)
```
...
```{r sort_gdevrel_high, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r sort_gdevrel_low, eval=TRUE, echo = TRUE, message=FALSE, warning=FALSE}
...
```{r sort_tdevrel_high, eval=TRUE, echo = TRUE, message=FALSE, warning=FALSE}
# Déviation générale - Déviations les plus élevées
df <- st_set_geometry(com, NULL)
...
```{r sort_tdevrel_low, eval=TRUE, echo = TRUE, message=FALSE, warning=FALSE}
# Déviation générale - Déviations les moins élevées
...
Ce nouveau contexte permet d'évaluer les pôles locaux d'emplois et permet ainsi indirectement de qualifier les principales discontinuités territoriales qui structurent l'espace d'étude : la fonction `discLayer` du package `cartoraphy`, permet ainsi d'extraire les discontinuités territoriales (max/min du ratio de référence) les plus importantes (25 % des valeurs les plus élevées). Les valeurs maximales et minimales des déviations spatiales (top 10) sont aussi extraites et le label de leur commune respective représenté sur la carte.
...
```{r sort_sdevrel_high, eval=TRUE, echo = TRUE, message=FALSE, warning=FALSE}
# Déviation générale - Déviations les plus élevées
...
```{r sort_sdevrel_low, eval=TRUE, echo = TRUE, message=FALSE, warning=FALSE}
# Déviation générale - Déviations les moins élevées
...
```{r devabs_contib, eval=TRUE, message=FALSE, echo = TRUE, warning=FALSE}
...
# Cartographie
mta_theme(title = "Synthèse multiscalaire - Déviations < 80 (- 25 % sous la moyenne)")
mf_map(x = com, var = "mst", type = "typo", border = "white", lwd = 0.2,
       pal = cols, val_order = unique(com$mst), leg_pos = "n",
       add = TRUE)
mf_map(epci, col = NA, border = "black", lwd = 1, add = TRUE)
...
iris <- st_read("data/input.gpkg", layer = "iris", quiet=TRUE)
com <- st_read("data/input.gpkg", layer = "com", quiet = TRUE) 
...
system <- sessionInfo()$platform
os <- sessionInfo()$running
sessinfo <- as.data.frame(rbind(R, os, system))

list_package <- sessionInfo()$otherPkgs
packages <- as.data.frame(NULL)
for (i in 1:length(list_package)){
Package <- list_package[[i]]$Package
Version <- list_package[[i]]$Version
Source <- unlist(strsplit(list_package[[i]]$Built, ";"))[[1]]
packages <- as.data.frame(rbind(packages, cbind(Package, Version, Source)))
}

kableExtra::kable_styling(kable(sessinfo, col.names = NULL))
kableExtra::kable_styling(kable(packages))
