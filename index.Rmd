...
Pour rejouer la préparation des données - jointures attributaires, sélection des métropoles, agrégation par clé d'appartenance territoiriale - se reporter à ce [script R](https://github.com/rysebaert/mta_rzine/blob/main/prepa.R).
...
## Synthèse (table)
tmp <- read.csv("data/com_meta.csv", sep = ";")
ft(tmp, id_num = NULL)
```
...
## Synthèse (table)
tmp <- read.csv("data/iris_meta.csv", sep = ";")
ft(tmp, id_num = NULL)
```
...
<p class="center"> **Télécharger les données [<img src="figure/zip.png" width="50"/>](https://github.com/rysebaert/mta_rzine/blob/d8ad5e6e91f61ee1407b2a7e5505de95f8c71657/data.zip)**</p>  

Pour rejouer l'analyse, décompressez les données contenues dans l'archive [data.zip](https://github.com/rysebaert/mta_rzine/blob/d8ad5e6e91f61ee1407b2a7e5505de95f8c71657/data.zip) puis ouvrez les couches `com` et `epci` contenues dans le geopackage `data.gpkg`.   
...
# Import (après avoir téléchargé et décompressé les données)
